...
import { getDay, startOfMonth, addMonths, subMonths, isToday, format } from 'date-fns'
...
const CalendarComponent = ({ onPressDay }: ICalendarComponent) => {
  ...
  const updateDaysArray = (month: any) => {
    ...
    const newDaysArray: any = Array.from({ length: startDayOfWeek }, (_, i) => null);
  ...
  return (
    <Box>
      ...
      >
        {/** Header do Calendar */}
        {/** Criar Component custom de button */}
        <TouchableOpacity
          activeOpacity={0.8}
          style={{
            width: 30,
        ...
        >
          <Icon name="left" size={16} color="#fff" />
        </TouchableOpacity>
        <Text 
          fontSize={16}
          fontWeight='bold'
      ...
      >
        ...
        {daysArray.map((item, key) => (
          ...
          >
            <Text 
              color={isToday(new Date(today.getFullYear(), today.getMonth(), item)) ? "white" : "black400"}
              fontWeight='600'
              fontSize={16}
            >
              {item
